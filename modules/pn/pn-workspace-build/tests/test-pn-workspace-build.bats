...
@test "--show-nix-commands-only exits 0 without building" {
    run bash -c "
      source '${LIB_PATH%%:*}'
...
@test "--show-nix-commands-only prints darwin-rebuild build with flake path" {
    run bash -c "
      source '${LIB_PATH%%:*}'
      set -- --show-nix-commands-only
      cd '$TEST_DIR/workspace'
    ...
    "
    [ "$status" -eq 0 ]
    echo "$output" | grep -q "darwin-rebuild build --flake"
