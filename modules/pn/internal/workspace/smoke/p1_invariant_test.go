...
func TestSmoke_P1Invariant(t *testing.T) {
	...
	for _, vc := range verbs {
		...
		for _, repo := range repos {
			...
			p1AssertUnchanged(t, repo, vc.name, baseline[repo], now, r)
		}
	}
}
...
// local-branch NAME SET must be identical, but individual branch SHAs are NOT
// asserted — `rebase main` legitimately moves the set's feature-x branch (which
// lives in the shared ref store), and HEAD/working-tree/branch checks below
// already guarantee the primary's checked-out branch (main) is untouched.
func p1AssertUnchanged(t *testing.T, repo, verb string, want, got p1Snapshot, r scenarioResult) {
	t.Helper()
	...
	if strings.Join(want.branchNames, ",") != strings.Join(got.branchNames, ",") {
