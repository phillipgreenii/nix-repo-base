...
2. Verify the change as much as possible in the worktree (see [Completion Gate](#completion-gate) — and its worktree caveat: Tiers 2/3 validate the repo's canonical checkout, not this worktree).
...
**Worktree caveat — Tiers 2 and 3 are NOT worktree-aware for a single repo.** `pn workspace flake-check` and `pn workspace build` resolve every workspace repo's path as `<workspace_root>/<repo-name>` — always that repo's **canonical** clone — no matter which directory `pn` was invoked from. If you did the edit in a plain `git worktree` of the repo under test (the [Worktree Merge-Back Flow](#main-means-local-main--the-worktree-merge-back-flow) above, NOT a coordinated workforest set) and then run Tier 2/3 from inside that worktree, `pn` silently validates the **canonical checkout's** content for that repo — your worktree's changes are never evaluated, and the gate PASSES VACUOUSLY. `pn workspace info` run the same way confirms it: it prints the canonical path for every repo, never the worktree you are standing in. `PN_WORKSPACE_OVERRIDE_PATHS` does **not** rescue this — see its row in [Environment Variables](#environment-variables): it is parsed only by `modules/pn`'s own unit tests, not by any `build`/`apply`/`info` code path, so setting it has no effect here (re-verified against `modules/pn` @ main, bead pg2-9ova0; see repo-base's own `CLAUDE.md` "pn workspace: building ONE repo from a worktree" for the full citation).

- **Single repo, no coordinated set:** Tier 1 (`nix flake check`/`nix build` run directly inside the worktree) IS worktree-correct — it never routes through `pn`'s repo-path resolution. For a Tier 3-equivalent check, use the WORKAROUND in repo-base's `CLAUDE.md`: invoke `nix build .#darwinConfigurations.<host>.system` (or `darwin-rebuild build`) directly from the terminal repo, replicating pn's `--override-input <alias> git+file://<path>` flags by hand with the target repo's path swapped to the worktree path.
- **Cross-repo work:** use a coordinated workforest set (`pn workspace workforest add`) instead — see [Coordinated Workforest Sets](#coordinated-workforest-sets). Every verb there, including Tier 2/3, IS worktree-aware, because the whole set (not one repo) is rooted at set-relative worktree paths.
...
| `PN_WORKSPACE_OVERRIDE_PATHS` | (empty)                                             | Comma-separated `name=path` pairs, parsed by `workspace.ParseOverridePaths`. **Not wired into any CLI command as of this writing** — `build`/`apply`/`info` never read it and there is no `--override-path` flag; its only callers repo-wide are `modules/pn`'s own Go unit tests (fixture-path injection). Setting it has **no effect** on `pn workspace build/apply/info` (bead pg2-9ova0). |
