...
by vendoring a manually-synced copy into a local `./pg-pr-src` directory, not by sharing live.
...
| `bd` CLI client (shell out, capture, wrap, parse `{data:[...]}`)    | 5, across 3 repos                     | `agent-support/packages/pg-pr/pkg/beads/runner.go` (70 lines); `agent-support/packages/pr-pool/internal/beads/runner.go` (73 lines — its own header states it "copies pg-pr's Runner/CLIRunner pattern rather than importing pg-pr's heavy module"); `agent-support/packages/pb/internal/bd/bd.go` (120 lines); `support-apps/packages/activity-collector/internal/collector/beads/beads.go` (151 lines); `repo-base/modules/pg-go-mutate/pg-go-mutate-tui/internal/beads/beads.go` (27 lines) |
| `git` CLI wrapping (shell out + parse `log`/`diff`/`rev-parse`)     | 3, across 2 repos                     | `agent-support/packages/pg-pr/internal/gitlocal/gitlocal.go` (own Runner, `git log`/`git diff --numstat` parsing); `agent-support/packages/ccpool/internal/gitfacet/gitfacet.go` (rev-parse trio, soft-fail); `support-apps/packages/activity-collector/internal/collector/git/git.go` (own Runner, differently-delimited `git log` parsing)                                                                                                                                                   |
...
| SQLite WAL + `busy_timeout` + `PRAGMA user_version` migration idiom | 3 confirmed                           | `agent-support/packages/pg-pr/internal/store/migrate.go`; `agent-support/packages/pg-ccaudit/internal/store/store.go`; `agent-support/packages/ccpool/internal/store/store.go`                                                                                                                                                                                                                                                                                                                 |
| Sandboxed CLI contract-test harness                                 | 2, different APIs, same idea          | `agent-support/packages/pb/cmd/pb/contract_test.go`; `agent-support/packages/ccpool/cmd/ccpool/contract_harness_test.go`                                                                                                                                                                                                                                                                                                                                                                       |
| Exponential backoff                                                 | 1 clean, generic, unduplicated so far | `agent-support/packages/pr-pool/internal/backoff/backoff.go`                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
schema_version:N}`) parsing, and the common verbs every consumer already calls identically
  (`show`, `list`, `comment`, `update`). App-specific domain modeling (`pg-pr`'s
  mergerequest/deptree/adjudication types) stays local — that is business logic layered on top, not
  plumbing.
...
- **`sqliteutil`** — the WAL + `busy_timeout` DSN + `PRAGMA user_version` schema-versioning idiom
  shared by `pg-pr`, `pg-ccaudit`, and `ccpool`'s stores. `pa-monitor`'s and `pr-pool`'s own stores
  should be re-checked for the same idiom before this package's shape is finalized (§10).
- **`backoff`** — `pr-pool`'s existing generic `Policy{Initial, Factor, Max}` retry policy, moved
  as-is; nothing else duplicates it yet, so this is a low-risk, opportunistic move.
- **`testkit`** — the sandboxed CLI-integration-test harness pattern (isolate `$HOME`/env, invoke
  the binary under test, capture stdout+exit code), converging `pb`'s and `ccpool`'s independently-
  built (and differently-shaped) contract-test scaffolding. Imported only from `_test.go` files —
  never shipped in a production binary, so whether it belongs in the same module as the production
  packages above or a separate one is an open item (§10).
...
- **`sqliteutil`**: re-verify `pa-monitor`'s and `pr-pool`'s own stores for the same WAL/
  `busy_timeout`/`PRAGMA user_version` idiom before finalizing the package's shape — the survey
  confirmed 3 implementations but did not rule these two in or out.
