...
- [ ] **Step 1 — Author the Facade:** fork `pn-workspace-sync` → WORK `pnwf sync-fetch` (+ conflict resolution) → validate → land → cleanup → POST (from canonical): `pn workspace push`. (SUPERSEDED as written by [ADR 0023](../../adr/0023-workspace-push-owns-sibling-propagation.md): the POST `pn workspace update --siblings-only` step is gone — `update` is local-only and cannot publish; `pn workspace push` now owns the interleaved push + sibling relock.) Opening summary MUST state plainly that on success it **pushes every repo to `origin/main`** (authorized by invocation — no second gate). Reference the stage skills by name.
...
- [ ] **Step 5 — Producer→consumer relock (M4):** after repo-base lands, relock agent-support's repo-base input so agent-support's `flake.lock` points at the repo-base rev carrying `pnwf`. This step, and the applied delivery of `pnwf`, require pushing repo-base to `origin` (surface this to the user; pushing is a separate, user-authorized step). (SUPERSEDED as written by [ADR 0023](../../adr/0023-workspace-push-owns-sibling-propagation.md): the command is now `pn workspace push`, which pushes AND relocks consumers in topological order; `pn workspace update --siblings-only` no longer pushes.) Remove the set from canonical; re-run Tier-3 build on canonical `main` as a post-land recheck.
